...
ARG NODE_MAJOR='24'
ARG PNPM_VERSION='10'
ARG BASE_NODE_IMAGE="node:${NODE_MAJOR}-${BASE_OS_VERSION}"

########################
# NODE 
########################
FROM ${BASE_NODE_IMAGE} AS node-build
...
ARG NODE_MAJOR='24'
ARG PNPM_VERSION='10'
ARG BUILD_DEPENDENCY_PACKAGES_ALPINE='argon2-dev bash brotli-dev ca-certificates coreutils curl-dev git gnu-libiconv-dev libcap libsodium-dev cmake libstdc++ libxml2-dev linux-headers mailcap oniguruma-dev openssl-dev readline-dev sqlite-dev upx xz dcron'
ARG BUILD_DEPENDENCY_PACKAGES_DEBIAN='cmake curl git libargon2-dev libbrotli-dev libcap2-bin libcurl4-openssl-dev libonig-dev libreadline-dev libsodium-dev libsqlite3-dev libssl-dev libxml2-dev mailcap xz-utils zlib1g-dev ca-certificates cron'
...
# Install Node and pnpm for build/runtime tools
RUN set -eux; mkdir -p /usr/local/bin /usr/local/lib;
# Copy Node runtime artifacts from official Node images
...
COPY --from=frankenphp-build /usr/local/lib/node_modules /usr/local/lib/node_modules
